...
func (d *D13Prize) ResolvePart2() (int, int, bool) {
	minB := ((float64(10000000000000+d.Y) * float64(d.ButtonA.X)) - (float64(d.ButtonA.Y) * float64(10000000000000+d.X))) / ((-1 * float64(d.ButtonA.Y) * float64(d.ButtonB.X)) + (float64(d.ButtonA.X) * float64(d.ButtonB.Y)))

	if minB != math.Floor(minB) {
		return 0, 0, false
	}

	minA := (float64(10000000000000+d.X) - float64(d.ButtonB.X)*minB) / float64(d.ButtonA.X)
...
func (d *D13Prize) ResolvePart1() (int, int, bool) {
